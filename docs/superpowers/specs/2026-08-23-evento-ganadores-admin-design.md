...
El campo `rank` existe desde el principio para permitir varios ganadores por categoría (2º, 3º puesto) en el futuro **sin ninguna migración adicional** — no hay restricción `unique` sobre `(category_id, rank)` ni sobre `category_id` solo. La interfaz de administración inicial, sin embargo, solo permite gestionar un ganador por categoría (no expone la creación de una segunda fila); ampliar esto más adelante es un cambio de interfaz, no de esquema.
